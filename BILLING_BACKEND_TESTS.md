...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/patients"
```
...
```bash
curl -X POST "https://healthscan-parse.preview.emergentagent.com/api/invoices" \
...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/invoices/YOUR_INVOICE_ID_HERE"
```
...
```bash
curl -X POST "https://healthscan-parse.preview.emergentagent.com/api/payments" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/invoices/patient/YOUR_PATIENT_ID_HERE"
```
...
```bash
# Get unpaid invoices
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/invoices?payment_status=unpaid"

# Get invoices for date range
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/invoices?from_date=2025-10-01&to_date=2025-10-31"
```
...
```bash
curl -X POST "https://healthscan-parse.preview.emergentagent.com/api/claims" \
  -H "Content-Type: application/json" \
  -d '{
    "invoice_id": "YOUR_INVOICE_ID_HERE",
    "medical_aid_name": "Discovery Health",
...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/claims/YOUR_CLAIM_ID_HERE"
```
...
```bash
curl -X PATCH "https://healthscan-parse.preview.emergentagent.com/api/claims/YOUR_CLAIM_ID_HERE/status?status=submitted"
```
...
```bash
curl -X PATCH "https://healthscan-parse.preview.emergentagent.com/api/claims/YOUR_CLAIM_ID_HERE/status?status=approved&approved_amount=632.50"
```
...
```bash
# All claims
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/claims"

# Filter by status
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/claims?status=submitted"

# Filter by medical aid
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/claims?medical_aid=Discovery%20Health"
```
...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/reports/revenue?from_date=2025-10-01&to_date=2025-10-31"
```
...
```bash
curl -X GET "https://healthscan-parse.preview.emergentagent.com/api/reports/outstanding"
```
...
```bash
curl -X PATCH "https://healthscan-parse.preview.emergentagent.com/api/claims/YOUR_CLAIM_ID/status?status=rejected&rejection_reason=Invalid%20ICD10%20code&rejection_code=ERR001"
```
