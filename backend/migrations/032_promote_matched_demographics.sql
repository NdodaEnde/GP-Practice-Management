-- Migration 032: promote writes validated demographics to matched patients
--                 + reads medical_aid from its real (top-level) location
--
-- H1: when execute_action_promote_document MATCHED an existing patient it only
-- attached encounters/diagnoses/vitals and never updated the patient row, so a
-- reviewer's validated edits to phone/email/address/medical-aid on a returning
-- patient were silently dropped (the EHR kept stale first-visit data). Now a
-- matched patient is backfilled COALESCE(validated, existing): a non-empty
-- validated value wins, extraction NULLs never clobber existing data. Identity
-- columns (name/dob/id_number) are left untouched on a match.
--
-- M1: medical_aid was read as patient_demographics->>'scheme_name', but scheme
-- and member live at TOP-LEVEL extractions.medical_aid.* — so medical aid was
-- always dropped, even for new patients. Source corrected in both the INSERT
-- and the new matched UPDATE.
--
-- Verbatim copy of migration 031 with only those two changes. Applied by the
-- principal's explicit per-migration hand. Not auto-run.
...
BEGIN
    ...
    ELSE
        ...
        IF v_patient_id IS NULL THEN
            ...
            INSERT INTO patients (
                ...
                v_demo ->> 'address',
                COALESCE(
                    p_extractions -> 'medical_aid' ->> 'scheme_name',
                    p_extractions -> 'medical_aid' ->> 'name',
                    v_demo ->> 'medical_aid',
                    v_demo ->> 'scheme_name'
                )
            );
    ...
    -- H1: a matched (existing) patient must still receive the reviewer-
    -- validated demographics from THIS document. Without this, edits to
    -- phone/email/address/medical-aid on a returning patient were silently
    -- dropped and the EHR kept stale data. COALESCE(validated, existing):
    -- a non-empty validated value wins, but extraction NULLs never wipe
    -- data already on the record.
    IF v_patient_kind <> 'created' THEN
        UPDATE patients SET
            contact_number = COALESCE(NULLIF(btrim(COALESCE(v_demo ->> 'telephone_cell', v_demo ->> 'phone', '')), ''), contact_number),
            email          = COALESCE(NULLIF(btrim(COALESCE(v_demo ->> 'email', '')), ''), email),
            address        = COALESCE(NULLIF(btrim(COALESCE(v_demo ->> 'address', '')), ''), address),
            medical_aid    = COALESCE(NULLIF(btrim(COALESCE(COALESCE(
                    p_extractions -> 'medical_aid' ->> 'scheme_name',
                    p_extractions -> 'medical_aid' ->> 'name',
                    v_demo ->> 'medical_aid',
                    v_demo ->> 'scheme_name'
...
COMMENT ON FUNCTION execute_action_promote_document(TEXT, TEXT, JSONB, TEXT, TEXT, BOOLEAN) IS
    ...
    'FOR UPDATE NOWAIT mutual exclusion. Migration 031: comma-tolerant vitals. '
    'Migration 032: matched patients receive validated demographics; medical_aid '
    'read from top-level extractions.medical_aid.';
