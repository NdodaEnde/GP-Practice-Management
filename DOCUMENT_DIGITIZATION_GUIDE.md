...
```bash
# Upload a document
curl -X POST https://surgiscan-billing.preview.emergentagent.com/api/documents/upload-standalone \
...
# Match patient
curl -X POST https://surgiscan-billing.preview.emergentagent.com/api/documents/match-patient \
  -F "parsed_doc_id=abc-123" \
  -F "id_number=8503205555088"

# Link to existing patient (if match found)
curl -X POST https://surgiscan-billing.preview.emergentagent.com/api/documents/link-to-patient \
  -F "parsed_doc_id=abc-123" \
