...
### §D.1 The §3.2 test was specified vacuously against decision #2 (a second premise found underspecified — recorded so it is not re-derived)

Same register as §D, not failure narration: a premise that looked sound in the PR D plan and was found underspecified against the live registry, recorded so the next person inherits the knowledge and not the trap.

The PR D plan's §3.2 specified the highest-priority regression test as "drives the materialiser over a workspace holding an orphaned-source diagnosis." That sentence was reviewed and approved. The gap, present in the plan's own facts and unreconciled: the plan's Finding C named `patients_with_diagnosis_prefix` as the *diagnosis*-sourced probe, while **decision #2 registered exactly one production standing kind, `morning_briefing` → `patients_not_seen_since`, which is *encounter*-sourced.** "Drives the materialiser over a workspace holding an orphan" therefore did **not** entail "materialises an orphaned row": on `test-workspace-c9f4d540` the orphan is on a diagnosis, `patients_not_seen_since` returns 0 rows, the materialiser wrote nothing, and the test's sole assertion passed green having demonstrated nothing — **a false green in the highest-priority guard, inside the §2.4-signed close-out, caught at integration-premise-verification (verifying the premise before running to report), not at plan review.** Both facts (Finding C; decision #2) were in front of the reviewer and not composed — the same composition failure the test itself embodied.

The fix raised the test to the prose, never the prose to the test: a TEST-LOCAL diagnosis-sourced `StandingQuery` drives the *real* `materialise_standing_queries` over the orphaned workspace and asserts the **persisted `briefing_items` row** is `unresolvable`/`openable=False`/citation-carries-the-truncated-id, with the test's own teardown verified non-vacuous (the production registry is *asserted* back to exactly one kind, not assumed). The §E "code-path-proven via a test that drives the materialiser's inner per-workspace function" sentence was honestly *worded* and, until this fix, not *backed*; it is now backed. Lesson for the next maintainer: a test-specification sentence that names a workspace property ("holds an orphan") must be composed against what the *registered* query kinds actually source, or it can be vacuous while reading correct — verify the premise, not the wording.
